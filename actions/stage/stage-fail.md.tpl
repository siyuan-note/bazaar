...
请先修复下列问题，再提升清单字段 `version`，重新打包 `package.zip`，并发布（或更新）标记为 Latest 的 GitHub Release，确保其中包含新的 `package.zip`。无需另行提交 Pull Request；索引一般会在数小时内自动同步。

Please fix the issues listed below first, then bump the manifest `version`, rebuild `package.zip`, and publish (or update) a GitHub Release marked as Latest that includes the new `package.zip`. A separate pull request is not required; the index is usually synchronized automatically within a few hours.
