...
4. **等待检查与合并**
   ...
   如果 PR Check 工作流检查不成功，请根据检查结果进行修改，不要重新创建 PR。修好包仓库（例如更新 Latest Release / `package.zip`）后，定时任务约每 20 分钟会按活跃度复检带 `ci-failed` 标签的开放 PR（包仓 Release 有变化时优先；长期无变化则逐渐降低频率）并更新检查评论；维护者也可手动打 `Check` 标签或触发工作流立即复检。
