...
└──────────────────────────┬──────────────────────────────────┘
                           ...
                           ▼
┌─────────────────────────────────────────────────────────────┐
│  Pkg Check（检查单个集市包）· 包路径 `pkgcheck/`                │
│  · 输入：解压根目录 + OwnerRepo + Type + Mode + 可选上下文       │
...
| 模块 | 职责 | 不负责 |
| --- | --- | --- |
| **Pkg Check** | 对单个已解压包根做确定性检查，返回 `Result`；规则集中在 `pkgcheck/rules` | PR diff、并发、OSS、stage.json、发评论、下 Release |
...
2. **同一套检查器，规则强度可配置**  
   - **PR 模式**：更严（多余字段、首发 `name` 合法性、与仓库名一致等）  
   - **Stage 模式**：保底（必要文件 + 关键字段），保证索引可更新即可  

   逻辑不同处用参数/模式区分，代码集中在一处。
...
代码：`pkgcheck/`（对外）+ `pkgcheck/rules/`（规则与流水线）。入口：`pkgcheck.Check(Input) *Result`。
...
| `Type` / `Mode` | 包类型；`ModePR` 更严 / `ModeStage` 保底 |
...
| 命名 | 路径首尾空格、Windows 保留设备名；`name` 对齐旧 `ValidateName` |
| 元数据 | `name` / `url` / `version` / `author` / `readme` / `funding.custom`；PR 拒未知字段；`name` 跨包唯一（靠 `OccupiedNames`） |
| 主题 | 非白名单禁止 `theme.js` |
...
由 PR Check 把 Issues 格式化即可；正文直接使用 `MessageZh` / `MessageEn`（已含现象与改法），例如：
...
Required file "icon.png" is missing from the package.zip root. ...
...
| 设计模块 | 现状大致位置 | 演进方向 |
| --- | --- | --- |
| PR Check | `actions/check` + `.github/workflows/pr-check.yml` | 整体重写；调用 `pkgcheck`，负责 diff / 评论 / `OccupiedNames` |
| Stage | `actions/stage` + `.github/workflows/stage.yml` | 整体重写；调用 `pkgcheck`，传入 `OldName`/`OldVersion` |
| Pkg Check | **`pkgcheck/` + `pkgcheck/rules/`** | 骨架与主要规则已落地；随规范继续加 step |
...
1. ~~抽出 Pkg Check~~ → 已完成（Context 流水线 + Issue 中英字段 + 主要规则）  
2. 重写 Stage / PR Check，改为调用 `pkgcheck`（ModeStage / ModePR；PR 侧组装 `OccupiedNames`）  
3. 补齐 PR 流程规则与 Stage 失败记错；评论按 `Rule` + 中英 Message 渲染  
