...
| 设计模块 | 现状大致位置 | 演进方向 |
| --- | --- | --- |
| PR Check | `actions/check` + `.github/workflows/pr-check.yml` | 整体重写；调用 `check`，负责 diff / 评论 / `OccupiedNames` |
| Stage | `actions/stage` + `.github/workflows/stage.yml` | 整体重写；调用 `check`，传入 `OldName`/`OldVersion` |
...
2. 重写 Stage / PR Check，改为调用 `check.Check`（ModeStage / ModePR；PR 侧组装 `OccupiedNames`）  
