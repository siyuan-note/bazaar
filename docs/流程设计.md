...
| 设计模块 | 现状大致位置 | 演进方向 |
| --- | --- | --- |
| PR Check | `actions/check` + `.github/workflows/pr-check.yml` | 已调用 `check.Check(ModePR)` + Issue 评论；流程规则见待办 |
| Stage | `actions/stage` + `.github/workflows/stage.yml` | 已调用 `check.Check(ModeStage)` + OccupiedNames；失败汇总见待办 |
